...
class BottleneckTransform(nn.Sequential):
    ...
    def __init__(
        ...
        layers["a"] = GhostConv2dNormActivation(
            width_in, w_b, kernel_size=1, stride=1,  activation_layer=activation_layer
        )
        layers["b"] = Conv2dNormActivation(
            w_b, w_b, kernel_size=3, stride=stride, groups=g,  activation_layer=activation_layer
        )
        ...
        layers["c"] = GhostConv2dNormActivation(
            w_b, width_out, kernel_size=1, stride=1,  activation_layer=None
        )
    ...
    def __init__(
        ...
        if should_proj:
            self.proj = GhostConv2dNormActivation(
                width_in, width_out, kernel_size=1, stride=stride,  activation_layer=None
            )
...
def regnet_y_400mf(*, weights: Optional[RegNet_Y_400MF_Weights] = None, progress: bool = True, **kwargs: Any) -> RegNet:
    ...
    params = BlockParams.from_init_params(depth=13, w_0=24, w_a=36.44, w_m=2.49, group_width=8, se_ratio=0.25, **kwargs)
