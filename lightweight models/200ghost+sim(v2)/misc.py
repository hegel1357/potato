...
class GhostModule(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int = 1, stride: int = 1,
                 ratio: int = 2, dw_kernel_size: int = 3, padding: int = 0,
...
# 定義新的 GhostConv2dNormActivation 模組，去除 BatchNorm
class GhostConv2dNormActivation(nn.Sequential):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
        ...
        layers = [
            GhostModule(in_channels, out_channels, kernel_size=kernel_size, stride=stride,
                        ratio=ratio, dw_kernel_size=1, padding=padding, activation=activation_layer)
        ]

        # 不添加 BatchNorm 層
        super(GhostConv2dNormActivation, self).__init__(*layers)
...
class SimamAttention(torch.nn.Module):
    """
    此模組實現了 SIMAM（Simple Attention Module，簡單注意力模組）。
    SIMAM 根據空間位置上的標準差計算簡單的注意力圖，並將其應用於輸入的特徵圖上。

    參數:
        input_channels (int): 輸入圖像的通道數
        scale_activation (Callable[..., torch.nn.Module]): 注意力圖的激活函數，預設為 ``torch.nn.Sigmoid``
    """
    ...
    def _compute_attention(self, input: Tensor) -> Tensor:
        # 計算每個空間位置的通道均值與標準差
        mean = input.mean(dim=1, keepdim=True)
        std_dev = input.std(dim=1, keepdim=True)

        # 使用 SIMAM 方法計算注意力圖
        attention_map = -1 * (input - mean) ** 2 / (2 * (std_dev ** 2 + 1e-5))
        attention_map = self.scale_activation(attention_map)  # 套用激活函數（Sigmoid）

        return attention_map

    def forward(self, input: Tensor) -> Tensor:
        attention_map = self._compute_attention(input)
        return attention_map * input  # 與輸入進行逐元素相乘
